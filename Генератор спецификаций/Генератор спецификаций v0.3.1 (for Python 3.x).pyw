...
title = 'Генератор спецификаций v0.3.1'

################  Н А С Т Р О Й К И  ###########################################################
# Данные для заполнения основной надписи (чтобы ячейка не заполнялась, заменить значение на '')
Razrab = 'ТрындецЪ'
Prov = 'VLaD-Sh'
N_kontr = 'Helicoid'
Utv = 'Вират Лакх'
Organizaciya = 'http://vk.com/k_experts'

# Масса в ячейке "Примечание" для БЧ
mass_prim = True        # Показывать ли массу в ячейке Примечание(Да - True,  Нет - False)
do = ''                # Текст перед значением массы
posle = ' кг'          # Текст после значения массы
tochnost = 2            # кол-во знаков после запятой в значении массы

# Номера разделов СП
razdel_sp = {"Документация":5,
        "Комплексы": 10,
    ...
    def EditSpc():
        ...
        for i in OS_collection:
            ...
            if not OS_collection[i].doc:
                if OS_collection[i].razdel not in ("Прочие изделия", "Стандартные изделия", "Материалы"):
                ...
                if OS_collection[i].razdel == "Детали" and not isp:
                    iSpc.ksSetSpcObjectColumnText(1, 1, 0, 'БЧ')   # Формат для БЧ
                    if mass_prim:
                ...
                elif OS_collection[i].razdel == "Сборочные единицы" and not isp:
                    iSpc.ksSetSpcObjectColumnText(1, 1, 0, 'А4')   # Формат для Сб.ед.

            if OS_collection[i].razdel != "Документация":
        ...
        if os.path.exists( path_sp ):
            res = iApplication.MessageBoxEx('Спецификация с именем "%s %s.spw" уже имеется в папке. Перезаписать файл?' % (Dictionary[2].replace('$|', ''), Dictionary[1]), title, 4)
            if res == 6:
    ...
    if iDocument.DocumentType == 5:                                                 # если сборка
        ...
        if iParts7.Count:                                                           # если есть компоненты
            ...
            if not value_obozn:
                ...
                OS_collection[value_naimen.replace(' ', '')].razdel = 'Документация'
                Dictionary[2] = ''

            else:
                obozn = '$' + value_obozn.replace('$|', '').replace(' ', '')
                tip_d = "Сборочный чертеж"                                     # тип документа по умолчанию
                for tip in type_doc:
                ...
                OS_collection[obozn] =  OS(value_obozn.rstrip('СБ') + 'СБ', tip_d)                  # метим ключ словаря символом '$' для сборки имеющей обозначение
                OS_collection[obozn].razdel = 'Документация'
            ...
            for i in OS_collection:
                ...
                if i[0] == '$':                                                     # если ОС с Обозначением
                    ...
                    if OS_collection[i].razdel == 'Сборочные единицы':
                        list_obozn_sbed.append(i)                                   # заполняем список обозначений Сборочных единиц
                    elif OS_collection[i].razdel in ('Детали', 'Документация'):
            ...
            if len(List_not_razdel) >0:
                ...
                iApplication.MessageBoxEx('Генерация спецификации завершена', title, 64)

        else:
            iApplication.MessageBoxEx('Активная сборка не имеет компонентов!', title, 48)
    else:
        iApplication.MessageBoxEx('Активный документ должен быть сборкой!', title, 48)
