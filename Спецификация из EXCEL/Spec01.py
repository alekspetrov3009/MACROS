...
while stroka1<stroka2:
    stroka1=stroka1+1
    vivod=listEx.Cells(stroka1,1).value     # Обозначение
    vivod2=listEx.Cells(stroka1,2).value    # Наименование
    vivod3 = listEx.Cells(stroka1,3).value  # Количество
    vivod4 =listEx.Cells(stroka1,4).value   # Примечание
    if vivod==None:
        break
    else:
        print (vivod, vivod2,vivod3)
...
###################################################################################
        if vivod3==None:
            ...
            iTextItemParam.Init()
            iTextItemParam.iSNumb = 0
            iTextItemParam.s = vivod3               #"3"     kol-vo########################################################################################
            iTextItemParam.type = 0
            iTextItemFont = kompas6_api5_module.ksTextItemFont(iTextItemParam.GetItemFont())
            iTextItemFont.Init()
            iTextItemFont.bitVector = 4096
...
##########################################################################
        if vivod4==None:
            ...
            iTextItemParam.Init()
            iTextItemParam.iSNumb = 0
            iTextItemParam.s = vivod4    # ПРИМЕЧАНИЕ
            iTextItemParam.type = 0
            iTextItemFont = kompas6_api5_module.ksTextItemFont(iTextItemParam.GetItemFont())
            iTextItemFont.Init()
            iTextItemFont.bitVector = 4096
